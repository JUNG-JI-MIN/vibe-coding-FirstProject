...
class ShipScene
{
    ...
    {
        return ShipLayout::Clamp(v, lo, hi);
    }

    int CurrentDeck() const
    {
        return int(Clamp(std::floor((py + 4) / 4 + .5f), 0, 2));
    }
    ...
    void Move(float dx, float dz)
    {
        ...
        {
            float nextY = py;
            if (layout.Support(px + dx, pz, py, nextY) && layout.CanStand(px + dx, nextY, pz) &&
                !pipePuzzle.Blocks(px + dx, nextY, pz))
            {
                px += dx;
                py = nextY;
            }
            if (layout.Support(px, pz + dz, py, nextY) && layout.CanStand(px, nextY, pz + dz) &&
                !pipePuzzle.Blocks(px, nextY, pz + dz))
            {
                pz += dz;
                py = nextY;
            }
    ...
    void DrawHud()
    {
        ...
        glColor4f(.65f, .76f, .8f, 1);
        Text(x + 16, y + 23, "VITALS / ECG");
        char label[40];
        sprintf_s(label, "HP %03d", int(health));
        glColor4f(red, green, blue, 1);
        Text(x + 192, y + 23, label);
        glColor4f(.18f, .38f, .43f, .22f);
        glBegin(GL_LINES);
        for (int i = 0; i <= 12; ++i)
        {
            float gx = x + 16 + i * 20.f;
            glVertex2f(gx, y + 36);
            glVertex2f(gx, y + 100);
        }
        for (int i = 0; i < 5; ++i)
        {
            float gy = y + 36 + i * 16.f;
            glVertex2f(x + 16, gy);
            glVertex2f(x + 256, gy);
        }
        glEnd();
        ...
        glColor4f(red, green, blue, 1);
        Text(x + 16, y + 119,
             health <= 0 ? "NO SIGNAL" : (health < 30 ? "CRITICAL" : (health < 65 ? "CAUTION" : "STABLE")));
        // Entire stamina widget disappears at full charge, including its background.
        if (staminaAlpha > 0)
        {
            const float sx = 490, sy = 659;
            glColor4f(1, 1, 1, staminaAlpha * .75f);
            Text(sx, sy - 12, exhausted ? "RECOVERING" : "STAMINA");
        ...
        glColor4f(.8f, .82f, .84f, 1);
        Text(1066, 254, "EQUIPPED / HOLD TAB");
        Text(1066, 278, ToolWheel::Name(tools.Equipped()));
        Text(1066, 304, pipePuzzle.HasCard() ? "2F CARD: OWNED" : "2F CARD: MISSING");
        char charges[40];
        sprintf_s(charges, "MEDKIT CHARGES: %d", tools.Medkits());
        Text(1066, 326, charges);
        if (tools.Scanning())
        {
            float best = 10000;
            int index = 0;
            ...
            {
                float x, z;
                ColorQuest::World(i, 39, 43.5f, x, z);
            ...
            char scan[96];
            sprintf_s(scan, "SCAN / NEAREST CORE: %s / %.1f m / B1", ColorQuest::Name(index), best);
            glColor4f(.7f, .9f, .8f, 1);
            Text(22, 220, scan);
        }
        for (int i = 0; i < 4; ++i)
        {
            glColor4f(.72f, .8f, .85f, 1);
            char coreLabel[64];
    ...
    void DrawBlueprint()
    {
        ...
        if (size > width - 100)
        {
            size = Clamp(float(width) - 100, 100, 10000);
        }
        ...
        glColor4f(.24f, .29f, .31f, .22f);
        glBegin(GL_LINES);
        float grid = Clamp(scale * 2, 8, 30);
        for (float x = 0; x < width; x += grid)
        {
            glVertex2f(x, 68);
            glVertex2f(x, float(height - 50));
        }
        for (float y = 68; y < height - 50; y += grid)
        {
            glVertex2f(0, y);
            glVertex2f(float(width), y);
        }
        glEnd();
        ...
        for (int pass = 0; pass < 2; ++pass)
        {
            ...
            {
                if (b.deck != mapDeck || !b.solid || b.h < 3 ||
                    b.y - b.h * .5f > ShipLayout::DeckY(mapDeck) + .1f)
                {
                    continue;
                }
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            glColor4f(.7f, .8f, .79f, 1);
            glBegin(GL_LINE_STRIP);
            for (int j = 0; j < 4; ++j)
            {
                glVertex2f(x - 7 + j * 4, y + 6 - j * 4);
                glVertex2f(x - 3 + j * 4, y + 6 - j * 4);
            }
            glEnd();
        }
        if (mapDeck == 0)
        {
            ...
            {
                float wx, wz;
                ColorQuest::World(i, 39, 43.5f, wx, wz);
                float x = X(wx), y = Z(wz);
            ...
            glColor4f(.7f, .75f, .75f, 1);
            Ring(X(50), Z(50), 8);
            Text(X(50) + 12, Z(50) + 4, "HUB");
        }
        if (mapDeck == CurrentDeck())
        {
            float x = X(px), y = Z(pz), pulse = 1 + std::sin(glutGet(GLUT_ELAPSED_TIME) * .002f) * .05f;
            glColor4f(.75f, .82f, .79f, .45f);
            Ring(x, y, 26 * pulse);
            Ring(x, y, 31 * pulse);
            glColor4f(.3f, .95f, .42f, 1);
            glLineWidth(2);
            Ring(x, y, 5);
            glLineWidth(1);
            ...
            glBegin(GL_LINES);
            glVertex2f(x, y);
            glVertex2f(x + std::sin(a) * 17, y - std::cos(a) * 17);
            glEnd();
        }
        Panel(0, 0, float(width), 68, .02f, .025f, .03f, .98f);
        glColor4f(.73f, .79f, .8f, 1);
        Text(24, 27, "VESSEL / DECK SCHEMATIC");
        for (int i = 0; i < 3; ++i)
        {
            float x = width * .5f - 108 + i * 76;
            Panel(x, 13, 68, 32, i == mapDeck ? .22f : .065f, i == mapDeck ? .29f : .075f,
                  i == mapDeck ? .3f : .08f, 1);
            glColor4f(.78f, .83f, .83f, 1);
            Text(x + 24, 34, i == 0 ? "B1" : (i == 1 ? "1F" : "2F"));
        }
        glColor4f(.48f, .65f, .65f, 1);
        Text(24, 54, ShipLayout::DeckName(mapDeck));
        Text(width - 48, 90, "N");
        Panel(0, float(height - 50), float(width), 50, .02f, .025f, .03f, .98f);
        glColor4f(.62f, .72f, .72f, 1);
        Text(24, float(height - 29),
             "M CLOSE  |  , / . CHANGE DECK  |  GREEN: YOU  |  DIAMOND: CORE  |  STEPS: STAIRS");
        glDisable(GL_BLEND);
...
public:
    void Button(int button, int state)
    {
        ...
        {
            tools.Button(state == GLUT_DOWN, !overview && captured);
        }
    }

    void ReleaseGraphics()
    {
        metalMaterial.Release();
    ...
    void Key(unsigned char k, bool down)
    {
        ...
        if (overview && (k == ',' || k == '<'))
        {
            mapDeck = mapDeck > 0 ? mapDeck - 1 : 0;
        }
        if (overview && (k == '.' || k == '>'))
        {
            mapDeck = mapDeck < 2 ? mapDeck + 1 : 2;
        }
        if (k == 27)
        {
            captured = !captured;
    ...
    void Mouse(int x, int y)
    {
        ...
        yaw += dx * .12f;
        pitch = Clamp(pitch + dy * .12f, -80, 80);
        glutWarpPointer(width / 2, height / 2);
    }
    ...
    void Update()
    {
        ...
        float dt = Clamp((now - lastTime) / 1000.f, 0, .05f);
        lastTime = now;
        ...
        pipePuzzle.Update(dt, px, py, pz, yaw, pitch, (keys['e'] || tools.RepairHeld()) && active, active,
                          [this](float x, float y, float z)
                          {
                              // Prevent interactions through bulkheads, slabs or furniture.
                              for (int i = 1; i < 24; ++i)
                              {
                                  float t = i / 24.f, rx = px + (x - px) * t,
                                        ry = py + 1.65f + (y - py - 1.65f) * t, rz = pz + (z - pz) * t;
                                  for (const auto& b : layout.boxes)
                                  {
                                      if (!b.solid && !b.slab)
                                      {
                                      ...
                                      if (std::fabs(rx - b.x) < b.w * .5f &&
                                          std::fabs(ry - b.y) < b.h * .5f && std::fabs(rz - b.z) < b.d * .5f)
                                      {
                                          return false;
                                      }
                                  }
    ...
    void Draw()
    {
        ...
        {
            double top = .07;
            glFrustum(-top * aspect, top * aspect, -top, top, .1, 85);
        }
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        if (overview)
        ...
        else
        {
            ...
            glRotatef(yaw, 0, 1, 0);
            glTranslatef(-px, -py - 1.65f, -pz);
        }
        glEnable(GL_LIGHTING);
        glEnable(GL_LIGHT0);
        glEnable(GL_LIGHT1);
        GLfloat ambient[] = {.22f, .26f, .3f, 1};
        ...
        std::sort(nearest.begin(), nearest.end(),
                  [this](const ShipLayout::Lamp& a, const ShipLayout::Lamp& b)
                  {
        ...
        for (const auto& b : layout.boxes)
        {
            if (overview)
            {
                ...
                if (!b.slab && b.y - b.h * .5f > ShipLayout::DeckY(mapDeck) + 2.7f)
                {
                    continue;
                }
            }
            else
            {
                ...
                if (b.y + b.h * .5f < py - 4.2f || b.y - b.h * .5f > py + 7)
                {
                    continue;
                }
            }
            if (hdr)
            {
                metalMaterial.Use(b.metallic, b.roughness, b.emission, true);
            }
            if (b.emission > 0)
            {
                glDisable(GL_LIGHTING);
        ...
        if (!overview)
        {
            pipePuzzle.Draw(hdr, metalMaterial, px, py, pz);
        }
        if (overview)
        {
            glDisable(GL_LIGHTING);
            if (mapDeck == CurrentDeck())
            {
                glColor3f(1, .7f, .15f);
                glPushMatrix();
                glTranslatef(px, 12, pz);
                glutSolidSphere(.8, 12, 8);
                glPopMatrix();
                float a = yaw * .0174532925f;
                glBegin(GL_LINES);
                glVertex3f(px, 12, pz);
                glVertex3f(px + std::sin(a) * 3, 12, pz - std::cos(a) * 3);
                glEnd();
            }
            glColor3f(.5f, .85f, 1);
            for (const auto& room : layout.rooms)
            {
                if (room.deck == mapDeck)
                {
            ...
            for (const char* p = "STAIR W"; *p; ++p)
            {
                glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, *p);
            }
            glRasterPos3f(72, 12, 60);
            for (const char* p = "STAIR E"; *p; ++p)
            {
                glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, *p);
            }
            if (mapDeck == 0)
            {
                ...
                {
                    float x, z;
                    ColorQuest::World(i, 35, 48, x, z);
                    glRasterPos3f(x - 3, 12, z);
                    for (const char* p = ColorQuest::Name(i); *p; ++p)
                    {
                        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, *p);
                    }
                }
                glRasterPos3f(48, 12, 50);
                for (const char* p = "CORE HUB"; *p; ++p)
                {
                    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_10, *p);
                }
            }
        ...
        char title[128];
        sprintf_s(title, "VESSEL / %s / 100 x 100 m",
                  ShipLayout::DeckName(overview ? mapDeck : CurrentDeck()));
        Text(22, 28, title);
        ...
        else
        {
            ...
            glEnd();
            DrawHud();
            glColor3f(.95f, .72f, .65f);
            Text(420, 598, pipePuzzle.Prompt());
