...
public:
    ...
    bool LightOn() const
    {
        return equipped == 0 && flash;
    }

    bool RepairHeld() const
    {
        return !open && pressed && (equipped == 1 || equipped == 2);
    }

    bool Scanning() const
    {
        return scan > 0 && equipped == 3;
    }
    ...
    void Mouse(int x, int y, int w, int h)
    {
        ...
        if (float(h) / 720 < scale)
        {
            scale = float(h) / 720;
        }
        if (dx * dx + dy * dy < 70 * 70 * scale * scale)
        {
            hover = -1;
            return;
        }
    ...
    void DrawHeld(int w, int h)
    {
        ...
        glFrustum(-.07 * aspect, .07 * aspect, -.07, .07, .1, 10);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        glTranslatef(.43f, -.38f, -.95f);
        if (pressed)
        {
            glRotatef(std::sin(spin * .0174533f) * 6, 1, 0, 0);
        }
        glColor3f(.18f, .19f, .2f);
        Box(0, -.14f, .10f, .15f, .28f, .17f); // Gloved hand.
        ...
        {
            Box(0, .04f, 0, .24f, .18f, .35f);
            Box(.02f, -.1f, .08f, .11f, .27f, .11f);
            glPushMatrix();
            glTranslatef(0, .04f, -.30f);
            glRotatef(spin, 0, 0, 1);
            glColor3f(.7f, .72f, .74f);
            glutSolidCylinder(.025, .18, 6, 1);
            glPopMatrix();
        }
        else if (equipped == 2)
        {
            Box(0, .08f, 0, .045f, .55f, .045f);
            Box(-.07f, .34f, 0, .045f, .14f, .065f);
            Box(.07f, .34f, 0, .045f, .14f, .065f);
        ...
        else if (equipped == 3)
        {
            ...
            glColor3f(.08f, .35f, .29f);
            Box(0, .07f, .05f, .20f, .19f, .015f);
        }
        else
        {
            Box(0, .03f, 0, .28f, .25f, .14f);
            glColor3f(.8f, .8f, .8f);
            Box(0, .03f, .08f, .15f, .035f, .015f);
            Box(0, .03f, .08f, .035f, .15f, .015f);
        }
    ...
    void DrawMenu(int w, int h)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            glColor4f(shade, shade, shade, .97f);
        ...
        for (int i = 0; i < 16; ++i)
        {
            ...
            glVertex2f(mx + std::cos(a) * 4, my + std::sin(a) * 4);
        }
        glEnd();
        glDisable(GL_BLEND);
    }
