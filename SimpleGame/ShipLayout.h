...
public:
    ...
    static float DeckY(int deck)
    {
        return (deck - 1) * 4.f;
    }

    static const char* DeckName(int deck)
    {
        return deck == 0 ? "B1 / ENGINEERING" : (deck == 1 ? "1F / HABITATION" : "2F / OPERATIONS");
    }
...
private:
    ...
    {
        boxes.push_back(
            {x, y + base, z, w, h, d, r, g, b, solid, metal, rough, emission, buildingDeck, slab});
    }
    ...
    void Bulkhead(bool horizontal, float fixed, float lo, float hi, std::vector<float> doors)
    {
        ...
        for (float door : doors)
        {
            ...
            if (end > begin)
            {
                if (horizontal)
                {
                    Wall((begin + end) * .5f, fixed, end - begin, .3f);
                }
                else
                {
                    Wall(fixed, (begin + end) * .5f, .3f, end - begin);
                }
            }
            if (horizontal)
            {
        ...
        if (hi > begin)
        {
            if (horizontal)
            {
                Wall((begin + hi) * .5f, fixed, hi - begin, .3f);
            }
            else
            {
                Wall(fixed, (begin + hi) * .5f, .3f, hi - begin);
            }
        }
    }
    ...
    void Slabs(int deck)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            for (int j = 0; j < 5; ++j)
            {
                ...
                Add((x0 + x1) * .5f, -.15f, (z0 + z1) * .5f, x1 - x0, .3f, z1 - z0, .24f, .28f, .31f, false,
                    .8f, .58f, 0, true);
                if (deck < 3)
                {
                    SurfaceRect(x0, x1, z0, z1, base);
                }
            }
        }
    }
    ...
    void Furnish(float x, float z, int index)
    {
        ...
        if (buildingDeck == 0)
        {
            ...
            for (int i = -1; i <= 1; ++i)
            {
                Add(x + i * 1.3f, 3.25f, z, .12f, .12f, 8, .36f, .4f, .42f, false);
            }
        }
    ...
    void RoomBlock(float x0, float z0, int block)
    {
        ...
        for (float x : {x0, xc, x1})
        {
            for (float z : {z0, zc, z1})
            {
        ...
        for (int ix = 0; ix < 2; ++ix)
        {
            for (int iz = 0; iz < 2; ++iz)
            {
                ...
                if (block == 3 && ix == 1 && iz == 0)
                {
                    continue;
                }
                Furnish(x, z, ix + iz * 2);
                lamps.push_back({x0 + 8.75f + ix * 17.5f, base + 3.35f, z0 + 8.75f + iz * 17.5f});
                Add(x0 + 8.75f + ix * 17.5f, 3.42f, z0 + 8.75f + iz * 17.5f, 1.8f, .08f, .25f, .3f, .72f, .8f,
                    false, .1f, .5f, 4);
            }
        }
    }
    ...
    void Core(float x, float z, bool mirror)
    {
        ...
        for (int deck = 0; deck < 3; ++deck)
        {
            ...
            if (deck == 0)
            {
                Add(x + 5.9f, .6f, Z(7), 3, 1.2f, .12f, .35f, .4f, .42f);
            }
            if (deck == 2)
            {
                Add(x + 2.1f, .6f, Z(7), 3, 1.2f, .12f, .35f, .4f, .42f);
            }
        }
...
public:
    ShipLayout()
    {
        ...
        for (int deck = 0; deck < 3; ++deck)
        {
            ...
            base = DeckY(deck);
            Wall(50, .2f, 100, .4f);
            Wall(50, 99.8f, 100, .4f);
            Wall(.2f, 50, .4f, 100);
            Wall(99.8f, 50, .4f, 100);
            ...
            RoomBlock(12, 53, 2);
            RoomBlock(53, 53, 3);
            for (float axis : {9.f, 50.f, 91.f})
            {
                for (float t : {9.f, 25.f, 41.f, 59.f, 75.f, 91.f})
                {
                    lamps.push_back({axis, base + 3.35f, t});
                    if (t != axis)
                    {
                        lamps.push_back({t, base + 3.35f, axis});
                    }
    ...
    bool Support(float x, float z, float currentY, float& result) const
    {
        ...
        for (const auto& s : surfaces)
        {
            ...
            if (difference <= .24f && difference < best)
            {
                best = difference;
                result = y;
                found = true;
    ...
    bool CanStand(float x, float y, float z) const
    {
        ...
        if (x < 6.15f + radius || x > 93.85f - radius || z < 6.15f + radius || z > 93.85f - radius)
        {
            return false;
        }
        for (const auto& b : boxes)
        {
            if (!b.solid || b.y - b.h * .5f >= y + 1.8f || b.y + b.h * .5f <= y + .05f)
            {
                continue;
            }
            float nx = Clamp(x, b.x - b.w * .5f, b.x + b.w * .5f),
                  nz = Clamp(z, b.z - b.d * .5f, b.z + b.d * .5f);
            if ((x - nx) * (x - nx) + (z - nz) * (z - nz) < radius * radius)
            {
                return false;
            }
        }
        ...
        for (const auto& b : boxes)
        {
            if (b.solid || b.h < .19f || b.w < 2 || b.d < .29f)
            {
                continue;
            }
            float bottom = b.y - b.h * .5f, top = b.y + b.h * .5f;
            if (bottom > y + .25f && bottom < y + 1.8f && top > y + .25f && x > b.x - b.w * .5f - radius &&
                x < b.x + b.w * .5f + radius && z > b.z - b.d * .5f - radius && z < b.z + b.d * .5f + radius)
            {
                return false;
            }
        }
