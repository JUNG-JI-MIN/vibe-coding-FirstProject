...
class PipePuzzle
{
    ...
    float Random()
    {
        ...
        return (randomState >> 8) / 16777216.f;
    }
    ...
    {
        return float((x * 37 + y * 71 + x * y * 13) % 101) / 101.f;
    }
    ...
    void Pipe(float x, float y, float z, float dx, float dy, float dz, float radius)
    {
        ...
        glPushMatrix();
        glTranslatef(x, y, z);
        if (std::fabs(dz) < length - .001f)
        {
            glRotatef(std::acos(dz / length) * 57.29578f, -dy, dx, 0);
        }
        else if (dz < 0)
        {
            glRotatef(180, 1, 0, 0);
        }
        glutSolidCylinder(radius, length, 12, 1);
        glPopMatrix();
    }
...
public:
    ...
    bool Blocks(float x, float y, float z) const
    {
        ...
        if (state != Open && std::fabs(x - 39) < 1.44f && std::fabs(z - 47) < .40f)
        {
            return true;
        }
        if (std::fabs(z - 48) < .40f && x > 32.6f && x < 39.4f)
        {
            return true;
        }
        return false;
                ...
                Visible visible)
    {
        ...
        auto Aim = [&](float x, float y, float z)
        {
            ...
            if (!active || py > -2.8f || distance > 2.4f || distance < .001f)
            {
                return false;
            }
            float dot =
                (dx * std::sin(ya) * std::cos(pa) - dy * std::sin(pa) - dz * std::cos(ya) * std::cos(pa)) /
                distance;
            return dot > .88f && visible(x, y, z);
        };
        if (Aim(33, -2.6f, 48.25f))
        {
            target = 1;
        }
        if (Aim(35, -2.55f, 48))
        ...
        if (state == Closed && target == 2 && e)
        {
            repair = Clamp(repair + dt / 3, 0, 1);
            if (repair >= 1)
            {
                state = Repaired;
            }
        ...
        float rate =
            state == Leaking ? (std::sin(clock * 5) > 0 ? 100.f : 25.f) : (state == Flowing ? 100.f : 0.f);
        spawn += rate * dt;
        while (spawn >= 1)
        {
            Emit(state == Flowing);
    ...
    const char* Prompt() const
    {
        if (target == 1)
        {
            return state == Leaking
                       ? "E  CLOSE VALVE"
                       : (state == Closed
                              ? "VALVE CLOSED / E REOPEN"
                              : (state == Repaired ? "E  OPEN REPAIRED PIPE" : "PRESSURE ROUTED TO DOOR"));
        }
        if (target == 2)
        {
            return state == Leaking ? "CLOSE THE VALVE BEFORE REPAIR"
    ...
    void Draw(bool hdr, MetalMaterial& material, float px, float py, float pz, bool particlesPass = false)
    {
        ...
        if (py > 1 || std::fabs(px - 35) > 60 || std::fabs(pz - 48) > 60)
        {
            return;
        }
        if (!particlesPass)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                glColor3f(.55f, .08f, .06f);
                Pipe(36, -.85f, z, .65f, 0, 0, .18f);
                glColor3f(.55f, .62f, .56f);
            }
            ...
            glColor3f(.25f, .34f, .25f);
            glPushMatrix();
            glTranslatef(33, -2.6f, 48);
            glutSolidSphere(.25, 12, 8);
            glPopMatrix();
            Pipe(33, -2.6f, 48, 0, 0, .28f, .06f);
            glColor3f(.7f, .08f, .055f);
            glPushMatrix();
            glTranslatef(33, -2.6f, 48.3f);
            glRotatef(wheel * 270, 0, 0, 1);
            glutSolidTorus(.035, .26, 8, 20);
            Cube(0, 0, 0, .50f, .035f, .04f);
            Cube(0, 0, 0, .035f, .5f, .04f);
            glPopMatrix();
            ...
            {
                glColor3f(.025f, .008f, .012f);
                Cube(35, -2.50f, 48.20f, .18f, .07f, .04f);
            }
            ...
            if (state != Open)
            {
                ...
                for (int i = 0; i < 12; ++i)
                {
                    for (int j = 0; j < 16; ++j)
                    {
                        ...
                        if (dissolve > 0 && threshold < dissolve)
                        {
                            continue;
                        }
        ...
        std::sort(particles.begin(), particles.end(),
                  ...
                  {
                      return view[2] * a.x + view[6] * a.y + view[10] * a.z <
                             view[2] * b.x + view[6] * b.y + view[10] * b.z;
                  });
        for (const auto& p : particles)
        {
            ...
            glColor4f(.65f, .025f, .065f, alpha);
            glVertex3f(p.x, p.y, p.z);
            glColor4f(.22f, .005f, .025f, 0);
            for (int i = 0; i <= 12; ++i)
            {
                float a = i * 6.2831853f / 12, rx = std::cos(a) * size, uy = std::sin(a) * size;
                glVertex3f(p.x + view[0] * rx + view[1] * uy, p.y + view[4] * rx + view[5] * uy,
                           p.z + view[8] * rx + view[9] * uy);
            }
            glEnd();
        }
        glDepthMask(GL_TRUE);
